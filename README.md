# <div align="center">¡Hola! Mi nombre es Fran <img src="https://raw.githubusercontent.com/ABSphreak/ABSphreak/master/gifs/Hi.gif" width="30px" height="30px"></div>

<div align="center">
  <a href="https://github.com/DenverCoder1/readme-typing-svg">
    <img src="https://readme-typing-svg.herokuapp.com?font=Fira+Code&weight=600&size=24&pause=1000&color=3498DB&center=true&vCenter=true&width=600&lines=Desarrollador+Web+Full+Stack;Aprendiendo+y+mejorando+constantemente;C%C3%B3digo+limpio+y+bien+estructurado" alt="Typing SVG" />
  </a>
</div>

<br>

## 👨‍💻 Sobre mí

<img align="right" src="https://media.giphy.com/media/qgQUggAC3Pfv687qPC/giphy.gif" width=30% style="border-radius: 15px;">

¡Hola! Soy **Fran**, un desarrollador web full-stack junior apasionado por crear soluciones digitales que marquen la diferencia. 

Con una mentalidad orientada a la resolución de problemas, creo que el software es la herramienta más poderosa para transformar ideas en realidad. Me encanta explorar nuevas tecnologías y mantenerme al día con las últimas tendencias del desarrollo web.

### 🌟 Lo que me motiva
- 🚀 **Innovación constante**: Siempre busco formas creativas de resolver desafíos técnicos
- 💡 **Código de calidad**: Creo en escribir código limpio, mantenible y escalable
- 🎯 **Aprendizaje continuo**: El mundo tech evoluciona rápido, ¡y yo evoluciono con él!
- 🤝 **Colaboración**: Me encanta trabajar en equipo y aprender de otros desarrolladores

### 🎯 Mis objetivos
- Convertirme en un desarrollador senior especializado en arquitecturas modernas
- Contribuir a proyectos open source significativos
- Explorar el mundo del desarrollo móvil y cloud computing

### 📍 Ubicación
## 💼 Experiencia

### 🚀 Proyectos Personales y Freelance
- **Desarrollador Web Full-Stack** | *Actualidad*
  - Desarrollo de aplicaciones web modernas utilizando React, Next.js y Node.js
  - Creación de APIs RESTful con Express.js y bases de datos SQL/NoSQL
  - Implementación de diseños responsivos con Tailwind CSS y Astro

### 🎓 Formación
- **Bootcamp de Desarrollo Web Full-Stack** | *2023 - 2024*
  - Tecnologías: JavaScript, React, Node.js, Express, MySQL, MongoDB
  - Proyecto final: Aplicación de gestión de eventos con autenticación

### 📚 Aprendizaje Continuo
- **Certificaciones**:
  - AWS Cloud Practitioner Essentials
  - Google IT Support Professional Certificate
- **Cursos actuales**:
  - Advanced React Patterns (Platzi)
  - System Design Interview (Frontend Masters)

### 🏆 Logros Destacados
- ✅ **Portfolio personal** creado con Next.js y desplegado en dominio personalizado
- ✅ **Aplicación de meetups** funcional con frontend y backend
- ✅ **Contribuciones** a proyectos open source

## 🛠️ Tecnologías & Herramientas

### 📊 Nivel de Experiencia
- 🔥 **Experto**: Uso diario, proyectos complejos
- ⚡ **Avanzado**: Uso frecuente, buena comprensión
- 📈 **Intermedio**: Uso regular, conocimientos sólidos
- 🌱 **Principiante**: Aprendiendo, primeros proyectos

### 💻 Stack Tecnológico

| Categoría | Tecnologías | Nivel |
|-----------|-------------|-------|
| **Lenguajes** | JavaScript, TypeScript, HTML5, CSS3 | ⚡ Avanzado |
| **Frontend** | React, Next.js, Astro, Tailwind CSS | 🔥 Experto |
| **Backend** | Node.js, Express.js, Bun | ⚡ Avanzado |
| **Bases de Datos** | MySQL, Turso, Supabase, Drizzle ORM | 📈 Intermedio |
| **Herramientas** | Git, GitHub, VS Code, ESLint, Prettier | 🔥 Experto |
| **Plataformas** | Vercel, Netlify | 📈 Intermedio |

### 🏆 Tecnologías Destacadas

<div align="center">

#### Frontend
<p>
  <img src="https://img.shields.io/badge/React-20232A?style=flat&logo=react&logoColor=61DAFB" alt="React">
  <img src="https://img.shields.io/badge/Next.js-000000?style=flat&logo=nextdotjs&logoColor=white" alt="Next.js">
  <img src="https://img.shields.io/badge/Astro-FF5D01?style=flat&logo=astro&logoColor=white" alt="Astro">
  <img src="https://img.shields.io/badge/Tailwind_CSS-38B2AC?style=flat&logo=tailwind-css&logoColor=white" alt="Tailwind">
</p>

#### Backend & Bases de Datos
<p>
  <img src="https://img.shields.io/badge/Node.js-339933?style=flat&logo=nodedotjs&logoColor=white" alt="Node.js">
  <img src="https://img.shields.io/badge/Express.js-000000?style=flat&logo=express&logoColor=white" alt="Express">
  <img src="https://img.shields.io/badge/MySQL-005C84?style=flat&logo=mysql&logoColor=white" alt="MySQL">
  <img src="https://img.shields.io/badge/Supabase-3ECF8E?style=flat&logo=supabase&logoColor=white" alt="Supabase">
</p>

#### Herramientas de Desarrollo
<p>
  <img src="https://img.shields.io/badge/TypeScript-007ACC?style=flat&logo=typescript&logoColor=white" alt="TypeScript">
  <img src="https://img.shields.io/badge/Git-F05032?style=flat&logo=git&logoColor=white" alt="Git">
  <img src="https://img.shields.io/badge/VS%20Code-007ACC?style=flat&logo=visual-studio-code&logoColor=white" alt="VS Code">
  <img src="https://img.shields.io/badge/ESLint-4B32C3?style=flat&logo=eslint&logoColor=white" alt="ESLint">
</p>

</div>

### 🚀 Tecnologías en Aprendizaje
<p align="center">
  <img src="https://img.shields.io/badge/Python-3776AB?style=flat&logo=python&logoColor=white" alt="Python">
  <img src="https://img.shields.io/badge/Docker-2CA5E0?style=flat&logo=docker&logoColor=white" alt="Docker">
  <img src="https://img.shields.io/badge/AWS-232F3E?style=flat&logo=amazonwebservices&logoColor=white" alt="AWS">
  <img src="https://img.shields.io/badge/GraphQL-E10098?style=flat&logo=graphql&logoColor=white" alt="GraphQL">
</p>

## 📊 Estadísticas GitHub

<div align="center">
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=efe13dev&theme=tokyonight&card_width=450em" alt="Top Languages" />
  <br><br>
  <img src="https://github-readme-stats.vercel.app/api?username=efe13dev&show_icons=true&theme=tokyonight&hide_title=true" alt="GitHub Stats" />
  <br><br>
  <img src="https://streak-stats.demolab.com?user=efe13dev&theme=tokyonight&hide_border=true" alt="GitHub Streak" />
</div>

## 🌟 Proyectos Destacados

### 🚀 **Meetups App** (Frontend + Backend)
<div align="center">
  <a href="https://github.com/efe13dev/meetups-frontend">
    <img src="https://github-readme-stats.vercel.app/api/pin/?username=efe13dev&repo=meetups-frontend&bg_color=23272E&title_color=F8FAFC&text_color=A5B4FC" alt="Meetups Frontend">
  </a>
  <a href="https://github.com/efe13dev/meetups-backend">
    <img src="https://github-readme-stats.vercel.app/api/pin/?username=efe13dev&repo=meetups-backend&bg_color=23272E&title_color=F8FAFC&text_color=A5B4FC" alt="Meetups Backend">
  </a>
</div>

**Aplicación full-stack para organizar y gestionar eventos meetup**

- 🏗️ **Tech Stack**: React, Node.js, Express, MySQL
- ✨ **Características**: Autenticación, gestión de eventos, registro de usuarios
- 🎯 **Estado**: En desarrollo activo


### 🃏 **Descubre Parejas**
<div align="center">
  <a href="https://github.com/efe13dev/descubre-parejas">
    <img src="https://github-readme-stats.vercel.app/api/pin/?username=efe13dev&repo=descubre-parejas&bg_color=23272E&title_color=F8FAFC&text_color=A5B4FC" alt="Descubre Parejas">
  </a>
</div>

**Juego de cartas interactivo con mecánicas de matching**

- 🛠️ **Tech Stack**: HTML, CSS3, JavaScript
- 💫 **Características**: Sistema de emparejamiento de cartas, animaciones fluidas, puntuación
- 🚀 **Estado**: Versión beta completada
- 🎪 **Innovación**: Mecánicas de juego intuitivas y diseño responsive

### 🍽️ **Qué Comemos Hoy**
<div align="center">
  <a href="https://github.com/efe13dev/QueComemosHoy">
    <img src="https://github-readme-stats.vercel.app/api/pin/?username=efe13dev&repo=QueComemosHoy&bg_color=23272E&title_color=F8FAFC&text_color=A5B4FC" alt="Qué Comemos Hoy">
  </a>
</div>

**App móvil para gestionar menús semanales con recetas propias**

- 🎨 **Tech Stack**: React Native, JavaScript
- 🔄 **Características**: Gestión de menús semanales, base de datos de recetas personales, planificación nutricional
- 📱 **Estado**: Completado y funcional

### 🌐 **Portfolio Personal**
<div align="center">
  <a href="https://efe-13.es/">
    <img src="https://img.shields.io/badge/Portfolio%20Personal-000000?style=for-the-badge&logo=nextdotjs&logoColor=white" alt="Portfolio">
  </a>
</div>

**Mi portfolio profesional creado con Next.js**

- 🚀 **Tech Stack**: Next.js, React, Tailwind CSS
- 📄 **Características**: Responsive design, animaciones suaves, SEO optimizado
- 🎯 **Estado**: Live en dominio personalizado
- 💼 **Objetivo**: Mostrar mis proyectos y habilidades de desarrollo

<br>

## 📫 Contacto & Conexión

<div align="center">

### 💼 **Conectemos profesionalmente**

| Plataforma | Contacto | Estado |
|------------|----------|---------|
| **LinkedIn** | [Francisco Lorca](https://www.linkedin.com/in/fcolorca/) | <img src="https://img.shields.io/badge/Disponible-brightgreen"> |
| **Email** | efe13dev@gmail.com | <img src="https://img.shields.io/badge/Respondiendo-blue"> |
| **GitHub** | [@efe13dev](https://github.com/efe13dev) | <img src="https://img.shields.io/badge/Activo-green"> |
| **Portfolio** | [efe-13.es](https://efe-13.es/) | <img src="https://img.shields.io/badge/Live-success"> |

### 📱 **Redes Sociales**
<p align="center">
  <a href="https://www.linkedin.com/in/fcolorca/" target="_blank">
    <img src="https://img.shields.io/badge/LinkedIn-0077B5?style=flat&logo=linkedin&logoColor=white" alt="LinkedIn">
  </a>
  <a href="https://github.com/efe13dev" target="_blank">
    <img src="https://img.shields.io/badge/GitHub-100000?style=flat&logo=github&logoColor=white" alt="GitHub">
  </a>
  <a href="mailto:efe13dev@gmail.com" target="_blank">
    <img src="https://img.shields.io/badge/Gmail-D14836?style=flat&logo=gmail&logoColor=white" alt="Gmail">
  </a>
</p>

### 🚀 **¿Quieres colaborar?**
- 💬 **Proyectos freelance**: Disponible para proyectos interesantes
- 🤝 **Colaboraciones open source**: Siempre abierto a contribuir
- 🎯 **Oportunidades laborales**: Buscando nuevos desafíos
- 📚 **Mentoring**: Me encanta compartir conocimientos

### 📝 **Nota**
¡No dudes en contactarme! Me encanta conocer a otros desarrolladores, discutir nuevas ideas y explorar oportunidades de colaboración. ¡Hablemos! 🚀

</div>

<br>

---

## 🎯 ¡Gracias por visitar mi perfil!

<div align="center">

### 💫 **¿Te gustó lo que viste?**

⭐ **¡Dame una estrella** en mis repositorios si encuentras algo útil

🔄 **Sígueme** para ver mis últimos proyectos y contribuciones

💬 **Conectemos** y charlemos sobre desarrollo web

### 🌟 **¿Buscas un desarrollador?**
- ⚡ **Rápido aprendizaje** de nuevas tecnologías
- 🎯 **Código limpio** y bien estructurado
- 🤝 **Trabajo en equipo** excelente
- 🚀 **Pasión por la innovación**

### 📈 **Estadísticas en tiempo real**
<img src="https://github-readme-stats.vercel.app/api?username=efe13dev&show_icons=true&theme=tokyonight&hide_title=true&include_all_commits=true" alt="GitHub Stats">

---

<div align="center">
  <i>"El código es como el humor. Cuando tienes que explicarlo, es malo." - Cory House</i>
  <br><br>
  <b>¡Feliz coding! 👨‍💻✨</b>
</div>

</div>
